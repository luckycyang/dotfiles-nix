...
let
  cfg = config.cyang.services.udev;
in
{
  options.cyang.services.udev = {
    enable = lib.mkOption {
      default = false;
      type = lib.types.bool;
  ...
  config = lib.mkIf cfg.enable {
    services.udev = {
      ...
      extraRules = ''
        ...
        # WCH Link Modify by ABoot
        ATTRS{idVendor}=="1a86", ATTRS{idProduct}=="8012", MODE="660", GROUP="plugdev", TAG+="uaccess"
